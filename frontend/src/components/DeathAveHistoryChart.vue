<template>
  <HistoryChartBase
    ...
    :user-id="userId"
    :data="data"
    :title="$t('statistics.sammary.deathAveHistory')"
    :label-y="$t('statistics.sammary.deathAve')"
    :width="width"
    :height="height"
  />
 </template>
...
import { calcDeathAveHistory } from '@/modules/Statistics'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const data = computed(() => {
      ...
      const history = calcDeathAveHistory(stats, rule, stage, buki)
      
      const ds_rate = {
        label: t('statistics.rateChart.DeathAveTotalLegend'),
        data: history.total,
      ...
      const ds_rate_daily = {
        label: t('statistics.rateChart.DeathAveDailyLegend'),
        data: history.daily,
