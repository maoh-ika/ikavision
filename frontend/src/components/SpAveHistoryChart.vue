<template>
  <HistoryChartBase
    ...
    :user-id="userId"
    :data="data"
    :title="$t('statistics.sammary.spAveHistory')"
    :label-y="$t('statistics.sammary.spAve')"
    :width="width"
    :height="height"
  />
 </template>
...
import { calcSpAveHistory } from '@/modules/Statistics'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const data = computed(() => {
      ...
      const history = calcSpAveHistory(stats, rule, stage, buki)
      
      const ds_rate = {
        label: t('statistics.rateChart.SpAveTotalLegend'),
        data: history.total,
      ...
      const ds_rate_daily = {
        label: t('statistics.rateChart.SpAveDailyLegend'),
        data: history.daily,
